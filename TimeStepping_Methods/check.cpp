...
void energychecking(vector<Star> & cluster)  // check virial energy condition 
{
	double kinetic = 0;
	double potential = 0;
	int exist = 0;
	for(int i=0;i<N;i++)
	{
		if( 1 )
		{
			kinetic += 0.5 *  double(1)/N * (pow((cluster[i]).tan_vel,2) + pow((cluster[i]).rad_vel,2));  
			for(int j = i+1;j<N;j++)
			{
				potential += (double(1)/N) / (cluster[j]).radius ;		// if radius of others larger than the star's				
			}
			potential += (i + 0.5)*(double(1)/N) / (cluster[i]).radius ;  // if radius of others smaller than the star's	
			exist++;
		}
